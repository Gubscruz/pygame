...
import tela_menu
import tela_help
from constantes import SAIR, TELA_INVENTARIO, TELA_JOGO, GAME_OVER, MENU, HELP
...
def jogo(janela, altura_tela, largura_tela):
    ...
    while estado['tela_atual'] != SAIR:
        ...
        if estado['tela_atual'] == TELA_JOGO:
            ...
            tela_inventario.desenha_inventario(janela, estado, altura_tela, largura_tela)
            tecla_apertada = motor_grafico.pega_tecla_apertada(janela)
            tela_inventario.atualiza_estado(estado, tecla_apertada)
        elif estado['tela_atual'] == GAME_OVER:
            tela_gameover.desenha_gameover(janela, estado, altura_tela, altura_tela)
            tecla_apertada = motor_grafico.pega_tecla_apertada(janela)
            tela_gameover.atualiza_estado(estado, tecla_apertada)
        elif estado['tela_atual'] == MENU:
            tela_menu.desenha_menu(janela, estado, altura_tela, altura_tela)
            tecla_apertada = motor_grafico.pega_tecla_apertada(janela)
            tela_menu.atualiza_estado(estado, tecla_apertada)
        elif estado['tela_atual'] == HELP:
            tela_help.desenha_help(janela, estado, altura_tela, altura_tela)
            tecla_apertada = motor_grafico.pega_tecla_apertada(janela)
            tela_help.atualiza_estado(estado, tecla_apertada)
